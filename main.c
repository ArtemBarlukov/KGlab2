...
// Нет необходимости в переменной texture и функциях, связанных с текстурой
...
TButton btn[] = {
    {"NumLab", {0,0, 100,0, 100,30, 0,30}, FALSE},
    {"FullName", {0,40, 100,40, 100,70, 0,70}, FALSE},
    {"Exit", {0,80, 100,80, 100,110, 0,110}, FALSE}
};
        ...
        case WM_LBUTTONDOWN:
            for (int i = 0; i < btnCnt; i++) {
                if (PointInButton(LOWORD(lParam), HIWORD(lParam), btn[i])) {
                    if (strcmp(btn[i].name, "NumLab") == 0) {
                        MessageBox(hwnd, "Лабораторная работа №2. Компьютерная графика", "Start", MB_OK | MB_ICONINFORMATION);
                        visible = TRUE;
                    }
                    if (strcmp(btn[i].name, "FullName") == 0)
                        MessageBox(hwnd, "Барлуков Артём Сергеевич. ЭВМб-22-1", "Start", MB_OK | MB_ICONINFORMATION);
                        visible = TRUE;
